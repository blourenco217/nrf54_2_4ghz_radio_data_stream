...
Data transmission from Peripheral to Central
=============================================

In addition to time synchronization, this sample demonstrates data transmission from the Peripheral to the Central device.
The Peripheral sends a data notification containing:

* A synchronized timestamp (8 bytes - in microseconds)
* A packet counter (4 bytes - incrementing counter)
* Dummy payload data (228 bytes - pattern-filled)

The total packet size is 240 bytes to efficiently utilize the BLE ATT MTU (247 bytes with overhead).
This demonstrates high-throughput data transmission over synchronized BLE connections.

The Central device receives this data and prints it to UART in the format:

.. code-block::

   DATA,timestamp=<microseconds>,counter=<integer>,size=<bytes>

This data can be read in real-time using the Python UART client provided in the ``client/`` directory.
See the :file:`client/README.md` for usage instructions.
...
#. Optionally, use the Python UART client to monitor real-time data transmission from Peripherals to Central:

   .. code-block:: console

      cd client
      python client_uart.py -p <serial_port>

   For example, on Windows: ``python client_uart.py -p COM3``

   See :file:`client/README.md` for more details on using the UART client.
...
.. note::
   In addition to the synchronization messages shown below, the Central will also print data notifications received from Peripherals in the format ``DATA,timestamp=<value>,value=<value>``.
...
* For the Peripherals:
  ...
  .. note::
     The Peripheral also sends data notifications back to the Central containing a timestamp and dummy integer value.
     These are printed on the Central device as ``DATA,timestamp=<value>,value=<value>``.
